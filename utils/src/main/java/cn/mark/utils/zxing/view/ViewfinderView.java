...
/***
 * @author marks.luo
 * @Description: TODO(该视图覆盖在相机预览的顶部, 它将取景器矩形和部分透明度添加到外部，以及激光扫描仪的动画和结果点。)
 * @date:2017-04-13 17:41
 */
public class ViewfinderView extends View {
    ...
    public ViewfinderView(Context context, AttributeSet attrs) {
        ...
        frameColor = array.getColor(R.styleable.ViewfinderView_frames_color, 0xFFFFFF);
...
//      canvas.drawRect(frame.left, scannerStart, frame.right, scannerStart + SCANNER_LINE_HEIGHT, paint);
    ...
    //处理颜色模糊
    private int shadeColor(int laserColor) {
        String hax = Integer.toHexString(laserColor);
