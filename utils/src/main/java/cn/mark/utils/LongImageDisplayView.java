...
import cn.mark.utils.longImageView.ImageSource;
import cn.mark.utils.longImageView.ImageViewState;
import cn.mark.utils.longImageView.OnStateChangedListener;
import cn.mark.utils.longImageView.decoder.CompatDecoderFactory;
import cn.mark.utils.longImageView.decoder.DecoderFactory;
import cn.mark.utils.longImageView.decoder.ImageDecoder;
import cn.mark.utils.longImageView.decoder.ImageRegionDecoder;
import cn.mark.utils.longImageView.decoder.SkiaImageDecoder;
import cn.mark.utils.longImageView.decoder.SkiaImageRegionDecoder;

/***
 * @author marks.luo
 * @Description: TODO(长图显示)
 * @date:2017-03-07 16:17
 */
@SuppressWarnings("unused")
public class LongImageDisplayView extends View {
    private static final String TAG = LongImageDisplayView.class.getSimpleName();
    ...
    public LongImageDisplayView(Context context) {
        this(context, null);
    }

    public LongImageDisplayView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        this.handler = new Handler(new Handler.Callback() {
            public boolean handleMessage(Message message) {
                if (message.what == MESSAGE_LONG_CLICK && onLongClickListener != null) {
                    maxTouchCount = 0;
                    LongImageDisplayView.super.setOnLongClickListener(onLongClickListener);
                    performLongClick();
                    LongImageDisplayView.super.setOnLongClickListener(null);
                }
                return true;
            }
        });
        ...
        if (attrs != null) {//获取自定义属性
            TypedArray typedAttr = getContext().obtainStyledAttributes(attrs, R.styleable.LongImageDisplayView);
            if (typedAttr.hasValue(R.styleable.LongImageDisplayView_assetName)) {
                String assetName = typedAttr.getString(R.styleable.LongImageDisplayView_assetName);
                if (assetName != null && assetName.length() > 0) {
                    setImage(ImageSource.asset(assetName).tilingEnabled());
                }
            }
            if (typedAttr.hasValue(R.styleable.LongImageDisplayView_src)) {
                int resId = typedAttr.getResourceId(R.styleable.LongImageDisplayView_src, 0);
                if (resId > 0) {
                    setImage(ImageSource.resource(resId).tilingEnabled());
                }
            }
            if (typedAttr.hasValue(R.styleable.LongImageDisplayView_panEnabled)) {
                setPanEnabled(typedAttr.getBoolean(R.styleable.LongImageDisplayView_panEnabled, true));
            }
            if (typedAttr.hasValue(R.styleable.LongImageDisplayView_zoomEnabled)) {
                setZoomEnabled(typedAttr.getBoolean(R.styleable.LongImageDisplayView_zoomEnabled, true));
            }
            if (typedAttr.hasValue(R.styleable.LongImageDisplayView_quickScaleEnabled)) {
                setQuickScaleEnabled(typedAttr.getBoolean(R.styleable.LongImageDisplayView_quickScaleEnabled, true));
            }
            if (typedAttr.hasValue(R.styleable.LongImageDisplayView_tileBackgroundColor)) {
                setTileBackgroundColor(typedAttr.getColor(R.styleable.LongImageDisplayView_tileBackgroundColor, Color.argb(0, 0, 0, 0)));
            }
            typedAttr.recycle();
        }
    ...
    private void doubleTapZoom(PointF sCenter, PointF vFocus) {
        ...
        float doubleTapZoomScale = Math.min(maxScale, LongImageDisplayView.this.doubleTapZoomScale);
    ...
    private static class TilesInitTask extends AsyncTask<Void, Void, int[]> {
        ...
        TilesInitTask(LongImageDisplayView view, Context context, DecoderFactory<? extends ImageRegionDecoder> decoderFactory, Uri source) {
        ...
        protected int[] doInBackground(Void... params) {
            try {
                ...
                LongImageDisplayView view = viewRef.get();
        ...
        protected void onPostExecute(int[] xyo) {
            final LongImageDisplayView view = viewRef.get();
            if (view != null) {
    ...
    private static class TileLoadTask extends AsyncTask<Void, Void, Bitmap> {
        private final WeakReference<LongImageDisplayView> viewRef;
        private final WeakReference<ImageRegionDecoder> decoderRef;
        private final WeakReference<Tile> tileRef;
        private Exception exception;

        TileLoadTask(LongImageDisplayView view, ImageRegionDecoder decoder, Tile tile) {
            this.viewRef = new WeakReference<>(view);
    ...
    private static class BitmapLoadTask extends AsyncTask<Void, Void, Integer> {
        ...
        protected void onPostExecute(Integer orientation) {
            LongImageDisplayView subsamplingScaleImageView = viewRef.get();
     ...
     * @return {@link AnimationBuilder} instance. Call {@link LongImageDisplayView.AnimationBuilder#start()} to start the anim.
     */
